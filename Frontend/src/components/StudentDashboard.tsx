...
function StudentDashboard() {
  ...
  useEffect(() => {
    async function fetchSubjects() {
      try {
        const url = `https://adrap-2.onrender.com/api/subjects/semester/${selectedSemester}/batch/${selectedBatch}/section/${selectedSection}`;
    ...
    const fetchSubjects = async () => {
      try {
        const response = await fetch(
          `https://adrap-2.onrender.com/api/subjects/student/${user.username}`
        );
  ...
  const handleLoadTest = async () => {
    ...
    try {
      ...
      const url = `https://adrap-2.onrender.com/api/studentTest?${queryParams.toString()}`;
      const response = await fetch(url);
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
  ...
  const handleSubmitMarks = async () => {
    ...
    try {
      const response = await fetch(
        "https://adrap-2.onrender.com/api/studentmarks/update",
        {
          method: "PUT",
