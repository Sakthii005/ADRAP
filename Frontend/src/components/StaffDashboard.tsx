...
function StaffDashboard() {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        const response = await fetch(
          `https://adrap-2.onrender.com/api/subjects/faculty/${user.username}`
        );
        ...
        // Set initial values from first available subject
        if (subjects.length > 0) {
          const firstSubject = subjects[0];
          setSelectedSemester(firstSubject.semester);
          setSelectedBatch(firstSubject.batch);
          setSelectedSection(firstSubject.section);
          setSelectedAcademicYear(firstSubject.academicYear);
  ...
  const semesterSubjects = React.useMemo(() => {
    ...
    const academicYearSubjects = facultySubjects.filter(
      (subject) => subject.academicYear === selectedAcademicYear
    );

    // Then filter by semester, batch, and section
    const filtered = academicYearSubjects.filter(
      (subject) =>
  ...
  useEffect(() => {
    // Only reset subject if it's not found in the filtered list
    const subjectExists = semesterSubjects.some(
  ...
  }, [selectedBatch, selectedSemester, selectedAcademicYear, selectedSection, semesterSubjects, selectedSubject]);
  ...
  useEffect(() => {
    const fetchExistingTest = async () => {
      ...
      try {
        const response = await fetch("https://adrap-2.onrender.com/api/serialtests");
        ...
        if (existingTest) {
          ...
          const exportRes = await fetch(
            `https://adrap-2.onrender.com/api/export/${existingTest.id}/export`
          );
          if (exportRes.ok) {
        ...
        if (typeof bstr === "string") {
          ...
          try {
            ...
            const testRes = await fetch(
              "https://adrap-2.onrender.com/api/serialtests",
              {
                method: "POST",
            ...
            const questionsRes = await fetch(
              "https://adrap-2.onrender.com/api/questions",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            const studentRes = await fetch(
              "https://adrap-2.onrender.com/api/studentmarks",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            const exportRes = await fetch(
              `https://adrap-2.onrender.com/api/export/${testId}/export`
            );
            if (exportRes.ok) {
              const exData = await exportRes.json();
              setExportData(exData);
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://adrap-2.onrender.com/api/serialtests/${testRecord.id}`,
        { method: "DELETE" }
      );
      if (!response.ok) {
  ...
  const handleRefreshStudent = async (rollNumber: string) => {
    try {
      ...
      const response = await fetch(
        `https://adrap-2.onrender.com/api/studentmarks/${testRecord.id}/${rollNumber}/reset`,
        {
          method: "POST",
          headers: {
      ...
      const exportRes = await fetch(
        `https://adrap-2.onrender.com/api/export/${testRecord.id}/export`
      );
    ...
    const fetchTestData = async () => {
      ...
      try {
        ...
        const testResponse = await fetch(
          `https://adrap-2.onrender.com/api/serialtests?subjectId=${selectedSubject}&serialTestNumber=${selectedTest}`
        );
        if (testResponse.ok) {
          ...
          if (testData) {
            ...
            const questionsResponse = await fetch(
              `https://adrap-2.onrender.com/api/questions/test/${testData.id}`
            );
